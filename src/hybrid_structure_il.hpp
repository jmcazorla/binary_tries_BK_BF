#ifndef HYBRID_STRUCTURE_IL_HPP
#define HYBRID_STRUCTURE_IL_HPP
...
#include "binTrie_ilRL.hpp"
using namespace std;
using namespace sdsl;

template <class selectType = select_support_mcl<1>, uint32_t block_size = 512, uint8_t low_part_size = 2>
class HybridStructure_il {
private:
    binTrie_ilRL<block_size> *binTrie;
    sdsl::int_vector<low_part_size> *lowPart;
    sdsl::bit_vector *flags;
    selectType f_select;

public:
    HybridStructure_il() = default;

    ~HybridStructure_il() = default;

    HybridStructure_il(std::vector<uint64_t>& set, uint64_t u) {
        uint64_t length_it = set.size();
        binTrie = new binTrie_ilRL<block_size>();
        ...
        HybridStructure_il::binTrie = new binTrie_ilRL<block_size>(highPart, newU);
        HybridStructure_il::f_select = selectType(flags);
    }
    ...
    inline binTrie_ilRL<block_size> *getBinTrie() {
        return binTrie;
    }
    ...
    inline uint64_t selectFlags(uint64_t value) {
        uint64_t select = HybridStructure_il::f_select(value);
        return select;
    }
    ...
    inline void encodeRuns() {
        uint64_t chain_length = (1 << HybridStructure_il::fixed_low_part_size);
        std::vector<uint64_t> * posOnes = new std::vector<uint64_t>(); // Vector to store the positions
        uint64_t number_ones = HybridStructure_il::binTrie->elements_coded();
        bool delete_prev = false;
        bool zero_prev = false;
        uint64_t pos_one = 1;
        uint64_t prev_one = HybridStructure_il::f_select(1);
        ...
        for (uint64_t i = 2; i <=  number_ones; ++i){
            pos_one = HybridStructure_il::f_select(i);
        ...
        vector<uint64_t> * onesDelLLevel = HybridStructure_il::binTrie->encodeRuns(posOnes);
